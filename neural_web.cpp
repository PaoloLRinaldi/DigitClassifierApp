...
void NeuralWeb::load_web(std::istream& is) {
  std::string s;
  getline(is, s);
  ...
  // Mi sa che queste due righe che seguono sono sbagliate.
  // No aspè perché sono sbagliate?
  // Io le lascio commentate ma non capisco perché
  // debbano essere sbagliate. No, secondo me
  // sono giuste, le decommento (mi riferisco a
  // "n_grads = 0;" e "gradients.clear();".
  n_grads = 0;
  gradients.clear();
}
...
void NeuralWeb::save_web(std::ostream& os) const {
...
void NeuralWeb::save_web(const std::string& file_name) const {
...
void NeuralWeb::calculate_gradient(const vector<double>& input,
  ...
  for (unsigned int i = nodes.size() - 1; i != 0; --i) {
    ...
    for (size_t j = 0; j != nodes[i].size(); ++j) {
      for (size_t k = 0; k != nodes[i - 1].size(); ++k) {
        ...
        grad.push_back(dCda_dadz[j] * hidden_layer_act(nodes[i - 1][k]) +
                       (i != nodes.size() - 1 ? tau * nodes[i][j][k] : 0));
      }
      grad.push_back(dCda_dadz[j]);
    }
  ...
  for (size_t i = 0; i != nodes[0].size(); ++i) {
    for (size_t j = 0; j != input.size(); ++j) {
      grad.push_back(dCda_dadz[i] * input[j]);
    }
    grad.push_back(dCda_dadz[i]);
  }
...
void NeuralWeb::update_weights_bias() {
  ...
  for (int i = nodes.size() - 1; i != -1; --i) {
    for (size_t j = 0; j != nodes[i].size(); ++j) {
      nodes[i][j].change_weights(avarage_grad.begin() + read_grad,
                                 avarage_grad.begin() + read_grad + n_nodes[i] + 1);
